...
p <- ggplot(data = world2) +
  ...
  ggplot2::geom_raster(data = l$loc_xy[l$pin==1 & l$loc_xy$yr%in%yrs_to_plot,], 
                       aes(x = x, y = y, fill= catch*encounter)) +
  ...
  ylab('') +
  xlab('') +
  geom_point(data = df[df$yr%in%yrs_to_plot &  !is.finite(df$val),],
             aes(x=utm_lon, y = utm_lat),
             color = "red",
             alpha = 1,
             shape = 3) +
  geom_point(data = df[df$yr%in%yrs_to_plot,],
