...
for(i in 1:4){
  p <- ggplot(data = world2) +
    ...
    ggplot2::geom_raster(data = na.omit(all[all_pin==1 & all$cat==cat[species],]), aes(x = x, y = y, fill= catch*encounter)) +
    ...
    ylab('') +
    xlab('') +
    geom_point(data = df[df$t_i%in%years_to_include & !is.finite(df$val) & df$c_iz == (species-1),],
               aes(x=utm_lon, y = utm_lat),
               color = "white",
               alpha = 0.5,
               shape = 3,
               size = 0.7)+
    geom_point(data = df[df$t_i%in%years_to_include & df$c_iz == (species-1),],
  ...
  ggsave(paste0("C:\\noaa\\projects\\juv_sablefish\\output\\Supplemental_",file_sp,".png"), p, width = 6, height = 10, units="in")
