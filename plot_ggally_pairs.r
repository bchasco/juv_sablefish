...
tmp <- data.frame(a = c(fit$Report$Epsilon1_sct[,1,]),
                  b = c(fit$Report$Epsilon1_sct[,2,]),
                  c = c(fit$Report$Epsilon1_sct[,3,]),
                  d = c(fit$Report$Epsilon1_sct[,4,]))
names(tmp) <- levels(as.factor(df$sp))
tmp$year <- c(rep('other',dim(fit$Report$Epsilon1_sct)[1]*22),
              rep(2020,dim(fit$Report$Epsilon1_sct)[1]))
